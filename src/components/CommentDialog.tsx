...
function CommentDialog({ interviewId }: { interviewId: Id<"interviews"> }) {
  ...
  const handleSubmit = async () => {
    if (!comment.trim()) return toast.error("Please enter comment ✍️");
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[600px]">
          ...
          {existingComments.length > 0 && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h4 className="text-sm font-medium">Previous Comments 🗣️</h4>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSubmit} className="bg-gradient-to-r from-teal-500 to-emerald-600 text-white font-medium rounded-lg shadow-md hover:shadow-lg hover:from-teal-600 hover:to-emerald-700 transition-all duration-300">Submit</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
